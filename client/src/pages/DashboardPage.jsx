...
import { buildCardSummary, buildICS, buildReturnMessage, copyToClipboard, downloadFile, minutesUntil, toDateInputValue, urgencyState } from '../utils/format';
...
export default function DashboardPage() {
  ...
  function buildMenuItems(task) {
    ...
    if (task.status === 'Pendente') items.push({ icon: IconCalendar, text:'Adicionar ao calendário (.ics)', action: () => downloadFile(`retorno-${task.nome.replace(/\s+/g,'-').toLowerCase()}.ics`, buildICS(task), 'text/calendar') });
    items.push({ sep:true });
