...
export default function DashboardPage() {
  ...
  function buildMenuItems(task) {
    const items = [];
    items.push({ text:'📋 Copiar mensagem de retorno', action: () => { copyToClipboard(buildReturnMessage(task)); showToast('Mensagem copiada.'); } });
    items.push({ text:'📋 Copiar resumo do card', action: () => { copyToClipboard(buildCardSummary(task)); showToast('Resumo copiado.'); } });
    items.push({ sep:true });
    if (task.status === 'Pendente') {
      items.push({ label:'Adiar' });
      items.push({ text:'+ 15 minutos', action: () => runAction(task.id, { tipo:'adiar_rapido', minutos:15 }) });
      items.push({ text:'+ 30 minutos', action: () => runAction(task.id, { tipo:'adiar_rapido', minutos:30 }) });
      items.push({ text:'+ 1 hora', action: () => runAction(task.id, { tipo:'adiar_rapido', minutos:60 }) });
      items.push({ text:'Amanhã (mesmo horário)', action: () => runAction(task.id, { tipo:'adiar_rapido', minutos:1440 }) });
      items.push({ text:'Adiar com motivo…', action: () => openActionDialog(task, 'adiar') });
      items.push({ sep:true });
    }
    items.push({ text:'📄 Duplicar retorno', action: () => handleDuplicate(task) });
    if (task.status === 'Pendente') items.push({ text:'📅 Baixar .ics (calendário)', action: () => downloadFile(`retorno-${task.nome.replace(/\s+/g,'-').toLowerCase()}.ics`, buildICS(task), 'text/calendar') });
    items.push({ sep:true });
    items.push({ text:'🗑 Excluir', danger:true, action: () => handleDelete(task) });
    return items;
  }
